...
var doneGames = $.map(games.done, function(n){return n});
var notyetGames = $.map(games.notyet, function(n){return n});
...
function collectNames(){
	var names = $("table.league tbody tr td:nth-child(3)").map(function(n, tr){return tr.textContent});
	var ret = [];
...
function collectTable(){
	var matrix=$("table.league tbody tr").map(function(n, tr){return $(tr.children).slice(5)});
	var ret = [];
...
function tableToGames(table){
	...
	for(var j=0; j<table[0].length; j++){
		...
		for(var i=0; i<table.length; i++){
			...
			if(round.every(function(g){return g[0]!=game[0]})) round.push(game);
		}
		if(notyetround){
			notyet.push(round);
		}else{
